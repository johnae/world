#!/usr/bin/env bash
# world CI router. Runs on `world.push` (see .cryo/README.md to apply).
#
# It posts the commit's `pending` status, fetches .cryo/ci.yaml at the
# pushed commit, and spawns it. The final success/failure comes from the
# shared `ci-status` handler, which watches the spawned run's lifecycle
# event and reads the `repo:` + `sha:` tags set below - so a build that
# fails after dispatch turns the commit red instead of leaving the
# dispatch-time green.
#
...
sha="$(first sha payload.body.after)"
ref="$(first ref payload.body.ref)"
repo_full="$(first repo payload.body.repository.full_name)"
echo "world-ci: repo=${repo_full} sha=${sha} ref=${ref}"
...
child="$(cryo spawn /tmp/world-run.yaml --tag "sha:${sha}" --tag "repo:${repo_full}" --tag world)"
