      ...
      function fish_user_key_bindings
        ...
        function skim-jump-to-project-widget -d "Show list of projects"
          begin
            ...
            set -lx dir (${pkgs.scripts}/bin/project-select ~/Development ~/.config)
            if [ "$dir" != "" ]
              cd $dir
        ...
        function kube-pod-exec -d "Exec into pod"
          begin
            ...
            set -lx SK_OPTS "--color=bw --no-hscroll --height=40"
            set -lx pod (${pkgs.kubectl}/bin/kubectl get pods --no-headers -o custom-columns=name:'{.metadata.name}' | "${pkgs.scripts}"/bin/sk-sk)
            if [ "$pod" != "" ]
