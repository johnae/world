{
  ...
  initialMasterNode = head (filter (node: node.config.services.k3s.settings.cluster-init) (lib.attrsets.attrValues clusterNodes));
  initialMaster = initialMasterNode.config.networking.hostName;
