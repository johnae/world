...
in {
  ...
  config = mkIf cfg.enable {
    ...
    services.coredns = {
      ...
      config = ''
        .:5300 {
            ...
            dns64 {
              prefix 64:ff9b::/96
              ## because blocky will use ipv4 - it's fine, we're ipv6 only
              allow_ipv4
            }
    ...
    ## NAT64 (Jool) configuration - runs in separate namespace (to allow the router itself access to ipv4 services)
    ...
    systemd.services.nat64-namespace-setup = mkIf cfg.enableNat64 {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "setup-nat64-namespace" ''
          ...
          # Add default IPv6 route in namespace to send response packets back to main
          ${pkgs.iproute2}/bin/ip netns exec nat64 ${pkgs.iproute2}/bin/ip -6 route replace default via fd00:64::1 dev veth-nat64-ns
