...
in {
  ...
  config = mkIf cfg.enable {
    ...
    networking.firewall.extraCommands = mkIf cfg.enableNat64 ''
      ...
      iptables -t nat -A POSTROUTING -s 198.51.100.0/30 -o ${cfg.externalInterface} -j MASQUERADE
    '';
    ...
    networking.firewall.extraStopCommands = mkIf cfg.enableNat64 ''
      ...
      iptables -t nat -D POSTROUTING -s 198.51.100.0/30 -o ${cfg.externalInterface} -j MASQUERADE 2>/dev/null || true
    '';
    ...
    systemd.services.nat64-namespace-setup = mkIf cfg.enableNat64 {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "setup-nat64-namespace" ''
          ...
          # IPv4: RFC 5737 TEST-NET-2 (internal use only, never touches real network)
