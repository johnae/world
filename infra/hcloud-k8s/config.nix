{lib, ...}: let
  ...
  createRandomMachineId = prefix: id: {
    "${prefix}-${toString id}" = {
      length = 4;
      upper = false;
      special = false;
...
resource.random_string = (lib.zipAttrs (lib.map (createRandomMachineId "master") masters)) //
(lib.zipAttrs (lib.map (createRandomMachineId "agent") agents));
