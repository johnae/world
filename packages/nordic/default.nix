# Stopgap: nixpkgs removed `nordic` as collateral of dropping the (GTK2-only)
# gtk-engine-murrine engine (NixOS/nixpkgs#544598). This vendors the upstream
# re-add-without-GTK2 fix (NixOS/nixpkgs#548486) so the Nord look keeps working
# on GTK3/4. Drop this directory once that PR lands in nixpkgs.
{
  lib,
  stdenvNoCC,
  fetchFromGitHub,
