      ...
      `(,(make-mu4e-context
        ...
        ,(make-mu4e-context
          ...
          :vars '(
                   ...
                   (smtpmail-smtp-server . "mail.9000.dev")
                   ;; gmail handles this
                   (mu4e-sent-messages-behavior . delete)
                 )
         )
...
#+END_SRC
Customize the built-in emacs project.el to also recognize folders with a .project file in them as projects
Taken from: https://christiantietze.de/posts/2022/03/mark-local-project.el-directories/ 
#+BEGIN_SRC elisp
 (defgroup project-local nil
  "Local, non-VC-backed project.el root directories."
  :group 'project)

(defcustom project-local-identifier '(".project" "flake.nix")
